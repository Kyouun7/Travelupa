...
var selectedImageEntity: ImageEntity? = null
...
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val imageDao = Room.databaseBuilder(
            applicationContext,
            AppDatabase::class.java, "travelupa-database"
        ).build().imageDao()
        ...
        setContent {
            TravelupaTheme {
                Surface(
                    ...
                    AppNavigation(currentUser, firestore, imageDao)
                }
            }
        }
    }
                    ...
                    try {
                        ...
                        val message = when {
                            e.message?.contains("CONFIGURATION_NOT_FOUND") == true -> 
                                "Error konfigurasi Firebase. Silakan hubungi administrator."
                            e.message?.contains("email address is already in use") == true -> 
                                "Email sudah terdaftar. Silakan gunakan email lain."
                            e.message?.contains("network") == true -> 
                                "Tidak ada koneksi internet. Periksa koneksi Anda."
                            else -> 
                                "Registrasi gagal: ${e.localizedMessage ?: "Unknown error"}"
                        }
    ...
    LaunchedEffect(Unit) {
        firestore.collection("tempat_wisata")
            .addSnapshotListener { snapshots, error ->
                ...
                val tempats = snapshots?.documents?.mapNotNull { doc ->
                    doc.toObject(TempatWisata::class.java)
                } ?: emptyList()
                daftarTempatWisata = tempats
            }
    }
...
fun TempatItem(tempat: TempatWisata) {
    Card(
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            Image(
                painter = painterResource(id = tempat.gambarResId ?: R.drawable.ic_launcher_foreground),
                ...
                DropdownMenu(
                    ...
                    DropdownMenuItem(onClick = {
                        ...
                        firestore.collection("tempat_wisata").document(tempat.nama)
                            ...
                            .addOnFailureListener { e ->
                                Log.e("TempatItemEditable", "Error deleting document", e)
                            }
                    }) {
                        Text("Delete")
                    }
